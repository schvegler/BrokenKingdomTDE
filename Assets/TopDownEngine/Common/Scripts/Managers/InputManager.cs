...
namespace MoreMountains.TopDownEngine
{
	...
	public class InputManager : MMSingleton<InputManager>
	{
		...
		/// Self Added script to controll the movement if the ai adds a target <--------------------------------------------------

		//Bool ti check if the character is controlled by the user or by AI
		bool uiControlled = false;

		//Reference to the player
		GameObject player;

		//Used as disatnce enemy/player to stop - controlled by MelleeWeapon.cs
		[SerializeField]
		float weaponDistance = 1;

		//Reference for the target for AI to move in the direction
		[SerializeField]
		Transform target;

		public void ToggleUIControll(bool value) {
		...
		public virtual void SetMovement(Vector2 movement)
		{
			if (IsMobile && InputDetectionActive)
			{
				...
				else //ZOMI < -------------------------------------------------- --------------------------------------------------
				{
					if (player != null)
					{
                        if (player.GetComponentInChildren<AIBrain>().Target != null)
                        {
                            target = player.GetComponentInChildren<AIBrain>().Target;
                        }
                    }
                    else
                {
                ...
                if (target == null)
                    ...
                    } else
                    {
						...
						_primaryMovement.x = directionToTarget.x;
						_primaryMovement.y = directionToTarget.z;

						if (Mathf.Abs(player.transform.position.x - target.position.x) < weaponDistance)
						{
							SetHorizontalMovement(0f);
						}

						if (Mathf.Abs(player.transform.position.z - target.position.z) < weaponDistance)
						{
							SetVerticalMovement(0f);
						}
					}
		...
		//Function to clear target if the user takes over the character control
		public void ClearTarget() {
			target = null;
		}

		public void SetWeaponDistance(float value) {
			if (value > 1)
            {
				weaponDistance = value;
			} else
            {
				weaponDistance = 1.2f;
            }
